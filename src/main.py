...
def process_single_image(image_path, report_id, debug=False):
    ...
    left_box_count_per_layer = box_counter.count_boxes_per_layer(left_boxes_per_stack, "", left_structure['avg_box_length'], left_structure['avg_box_width'], left_structure['stacking_type'], left_gap_in_inches)
    right_box_count_per_layer = box_counter.count_boxes_per_layer(right_boxes_per_stack, "", right_structure['avg_box_length'], right_structure['avg_box_width'], right_structure['stacking_type'], right_gap_in_inches)
    ...
    # TEMPORARY CHANGE: passing IMAGE_NAME as PART_NUMBER
    extra_left_box_count = box_counter.count_extra_boxes(left_structure['stacking_type'], left_structure['avg_box_length'], left_structure['avg_box_width'], left_structure['avg_box_height'], "", left_boxes, back_left_boxes, fartest_left_boxes, left_stack_count, left_pallet_status, left_box_count_per_layer)
    extra_right_box_count = box_counter.count_extra_boxes(right_structure['stacking_type'], right_structure['avg_box_length'], right_structure['avg_box_width'], right_structure['avg_box_height'], "", right_boxes, back_right_boxes, fartest_right_boxes, right_stack_count, right_pallet_status, right_box_count_per_layer)
    ...
    rds_insert_data.insert_record(image_name, report_id, rack_dict['Q3'], total_left_boxes, left_pallet_status, "NA", part_number, image_obj_key_id, exclusion="" if left_pallet_status != "N/A" else "empty rack")
    rds_insert_data.insert_record(image_name, report_id, rack_dict['Q4'], total_right_boxes, right_pallet_status, "NA", part_number, image_obj_key_id, exclusion="" if right_pallet_status != "N/A" else "empty rack")
